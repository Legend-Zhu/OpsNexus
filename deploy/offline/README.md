...
| 文件 | 部署位置 | 说明 |
|---|---|---|
| bundle/opsguard-server-1.0.0.tar | /opt/opsguard/images/ | 管理端镜像（本地构建导出） |
| bundle/opsguard-worker-1.0.0.tar | /opt/opsguard/images/ | Worker 镜像 |
...
| stack.yml | /opt/opsguard/stack.yml | Worker swarm 服务（8090，global） |
| ../../OpsGaurdWeb/deploy/config.docker.yaml | /opt/opsguard/server/config.yaml | 管理端配置（:8080、认证已开、registry 已开） |
| agent-config.disaster.yaml | 灾害集群 5 台 /etc/opsguard/agent-config.yaml | Worker 策略（webhook 注释待网络打通） |
| stack.disaster.yml | 232:/opt/opsguard/stack.disaster.yml | 灾害集群 Worker（6060，global，mode:host 发布） |

### 灾害集群（5 节点）部署记录与要点

1. 镜像 tar 两种格式：containerd 存储导出的 OCI 格式（27.3.1 可 load）与
   buildx/经典存储导出的 legacy 混合格式（20.10.24 可 load，`docker load` 认
   manifest.json + repositories）。向 20.10 节点分发必须用 legacy 格式。
...
```bash
# 本地（有网）
cd OpsGaurdWeb && docker build -t opsguard-server:1.0.0 -f deploy/Dockerfile .
cd Worker && docker build -t opsguard-worker:1.0.0 -f deploy/Dockerfile .
docker save -o opsguard-server-1.0.0.tar opsguard-server:1.0.0
docker save -o opsguard-worker-1.0.0.tar opsguard-worker:1.0.0
# 上传全部文件后，目标机：
bash /opt/opsguard/offline/install-docker.sh /opt/opsguard/offline/docker-27.5.1.tgz
docker load -i /opt/opsguard/images/opsguard-server-1.0.0.tar
docker load -i /opt/opsguard/images/opsguard-worker-1.0.0.tar
docker run -d --name opsguard-server --restart unless-stopped -p 8080:8080 \
  ...
  -v /var/run/docker.sock:/var/run/docker.sock opsguard-server:1.0.0
