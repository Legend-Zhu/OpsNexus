...
const modules = [
  ...
  { page: '集群', purpose: '接入与管理容器集群', actions: '接入集群;详情页含节点(CPU/内存实时)、服务部署/编辑/缩放/重启/日志、中间件、事件回溯、审计、告警规则、纳管配置' },
...
const scenes: Scene[] = [
  ...
  },
  {
    ...
    steps: [
      '集群详情页点「纳管配置」,在清单编辑器里逐条声明:名称(唯一)、类型(docker 容器 / 宿主机端口服务)、引用(容器名或 IP:端口)、所在节点(容器类型必填)、端口、分类(中间件/业务/基础设施/服务)、监控配置(端口探测/HTTP 检查)。',
    ],
    points: [
      '保存为整体替换当前清单;平台周期探测,只在状态翻转(异常/恢复)时产生事件进告警中心。',
      '带监控的纳管条目会自动出现在「告警规则」页签;清单里的容器在服务列表带「纳管」标签,仅有重启操作。',
    ],
  },
  {
